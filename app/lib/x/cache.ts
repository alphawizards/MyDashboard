...
async function ensureXCacheSchema(): Promise<boolean> {
  if (xCacheSchemaReady) return true;
  ...
  try {
    ...
    xCacheSchemaReady = rows !== null;
    return xCacheSchemaReady;
  } catch (err) {
...
export async function getCachedTweetsByAuthor(
  ...
  if (!(await ensureXCacheSchema())) return null;
...
export async function upsertTrackedAccounts(authors: readonly AuthorProfile[]): Promise<void> {
  ...
  if (!(await ensureXCacheSchema())) return;
